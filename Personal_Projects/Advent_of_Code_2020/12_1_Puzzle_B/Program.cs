...
namespace _12_1_Puzzle_B
{
    class Program
    {
        static int Solution(int toSum, int[] sortedNumbers)
        {
            ...
            while(sortedNumbers[left] + sortedNumbers[middle] + sortedNumbers[right] != toSum)
            {
                if(middle > right)
                {
                    return - 1;
                }
                if((sortedNumbers[left] + sortedNumbers[middle] + sortedNumbers[right] > toSum) && left > 0)
                {
                    left--;
                }
                else if((sortedNumbers[left] + sortedNumbers[middle] + sortedNumbers[right] > toSum) && left == 0)
                {
                    right--;
                    left = 0;
                    middle = 1;
